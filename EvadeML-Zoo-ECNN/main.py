...
def main(argv=None):
    ...
    print('Test accuracy on selected legitimate examples %.4f' % (accuracy_selected))#不是实验结论，验证选择样本是否都是cnn正确识别
    ...
    #图三第一行，存储攻击后的图片，每类存一个作为例子
    if FLAGS.visualize is True:
        from datasets.visualization import show_imgs_in_rows
        if FLAGS.test_mode or FLAGS.balance_sampling:#testmode是只取被正确分类的每一类图像的第一个作为test
